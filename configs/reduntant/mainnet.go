...
func (cfg *Config) GetToken(name string) (token string) {
	if token = cfg.Tokens[name]; token == "" {
		log.Fatal(token, " token not found.")
	}
	return token
}

func (cfg *Config) GetExchg(name string) (exchange string) {
	if exchange = cfg.Exchgs[name]; exchange == "" {
		log.Fatal(exchange, " exchange not found.")
	}
	return exchange
}
